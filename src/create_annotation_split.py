...
def create_annotation_splits(input_file, output_dir, overlap_ratio=0.15, random_state=42):
    ...
    # Tentukan kolom yang ada di paragraphs_df
    existing_columns = paragraphs_df.columns.tolist()
    
    # Tambahkan kolom yang diperlukan untuk anotasi jika belum ada
    for col in ['Sentimen', 'Topik_Utama', 'Keyword', 'Catatan']:
        if col not in existing_columns:
            paragraphs_df[col] = ''
    
    # Kolom untuk template anotator
    template_columns = ['ID_Dokumen', 'ID_Paragraf', 'Teks_Paragraf', 'Sentimen', 'Topik_Utama', 'Keyword']
    
    # Tambahkan kolom metadata tambahan
    ...
    for i, indices in enumerate([annotator_1_all, annotator_2_all, annotator_3_all], 1):
        annotator_data = paragraphs_df.loc[indices, template_columns].copy()
        ...
        # Tambahkan kolom Catatan jika belum ada
        if 'Catatan' not in annotator_data.columns:
            annotator_data['Catatan'] = ''
        else:
            annotator_data['Catatan'] = ''
